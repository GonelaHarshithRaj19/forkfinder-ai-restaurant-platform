...
const fetchNearbyRestaurants = async (lat, lng) => {
  ...
  // $near cannot be used inside countDocuments(), so use $geoWithin here.
  const existingCount = await Restaurant.countDocuments({
    location: {
      $geoWithin: {
        $centerSphere: [
          [lng, lat],
          MAX_DISTANCE / 6378100,
        ],
      },
    },
  ...
  if (existingCount > 0) {
    console.log(
      `[Cache HIT] ${existingCount} restaurants found in DB`
    );
  ...
  // Fetch fresh data from Overpass using GET
  console.log(
    `[Overpass] Fetching restaurants near ${lat}, ${lng}`
  );
  ...
  const response = await axios.get(url, {
    timeout: 30000,
    headers: {
      'User-Agent': 'ForkFinder/1.0 (portfolio project)',
    },
  });
  ...
  console.log(
    `[Overpass] Got ${nodes.length} raw nodes`
  );

  if (nodes.length === 0) {
    console.warn(
      '[Overpass] No restaurants returned — area may have sparse OSM data'
    );
  ...
  const unique = nodes.filter((node) => {
    const key =
      `${node.tags?.name || node.id}_` +
      `${node.lat?.toFixed(3)}_` +
      `${node.lon?.toFixed(3)}`;
  ...
  if (ops.length > 0) {
    ...
    console.log(
      `[MongoDB] Upserted ${ops.length} restaurants`
    );
  }

  // Return restaurants within 5km
  return Restaurant.find({
    location: {
      $near: {
        $geometry: {
